...
- If multiple exact-match candidates remain genuinely indistinguishable after the recency tie-breaker, stop the workflow and report to the user.
- Never treat a parser/extraction failure as evidence that there are zero candidates. If extraction is incomplete/corrupted or no deterministic existing session can be selected by name, stop the workflow and report to the user.
...
- Creating a new Claude session/chat is strictly forbidden in this workflow family.
...
- Always use continuation mode with explicit existing UUID:
