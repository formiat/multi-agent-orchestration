...
This workflow adds task-scoped branch preflight, investigation-specific OpenCode command usage, review-loop rules, and the final reporting contract on top of the shared delegated-agent and OpenCode-provider references.
...
- Before sending the first new OpenCode message in this workflow run, inspect the current branch for an already-produced investigation result from an earlier OpenCode round.
...
- When the user explicitly approves creating a new OpenCode session, bootstrap with:
  `opencode run --title "<codex_session_name>" --command investigate-bug <task_id>`
...
- inspect any user-provided local materials, logs, and trace files described in the prompt;
