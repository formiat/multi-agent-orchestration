---
description: Investigate a bug from freeform context and create or update INVESTIGATION.md.
agent: build
---
...
Read `./.codex/inbox.md` first and treat it as the authoritative request for this run. `$ARGUMENTS` may be empty or may repeat the same context; if both are present, `./.codex/inbox.md` wins.

Before writing your final response, clear `./.codex/outbox.md` so that file contains only the latest response. Do not rely on stdout for the final handoff; write the final English summary to `./.codex/outbox.md`.

The authoritative context may include:
...
After the investigation is committed, write to `./.codex/outbox.md` a concise English summary of the root cause hypothesis, strongest evidence, and open questions.
