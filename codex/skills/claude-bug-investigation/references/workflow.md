...
This workflow adds task-scoped branch preflight, investigation-specific prompt requirements, and the final reporting contract on top of the shared delegated-agent and Claude-provider references.
...
- If a non-empty `INVESTIGATION.md` relevant to the current task already exists, review it immediately.
- If it already meets the quality bar, stop and report without contacting Claude further.
- Otherwise keep the findings and use them as the first Claude inbox message instead of a fresh `/investigate-bug <task_id>` bootstrap or generic investigation request.
...
- When the user explicitly approves creating a new Claude session, bootstrap with a English request that tells Claude to `Выполни /investigate-bug <task_id>.`
...
- treat an existing `INVESTIGATION.md` as the current investigation state to refine or correct rather than ignore;
- inspect any user-provided local materials, logs, and trace files described in the inbox request;
...
- Study the task, inspect `INVESTIGATION.md`, and independently verify as many central claims as practical against the repository, local materials, current branch state, relevant tests, and git history.
