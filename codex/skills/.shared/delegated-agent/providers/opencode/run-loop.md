...
- Prefer capturing each `opencode run` stdout/stderr into a timestamped `/tmp/opencode_run_<session_id>_<timestamp>.log` file. Do not stream the full stdout/stderr into model context. On process completion, use this order: inspect outbox first; if outbox is missing/empty/insufficient, run a compact `rg` search over the captured log for service-cap and failure markers; read only a narrow tail as a last diagnostic step.
...
- If a run produces a non-empty outbox that is clearly unrelated to the active request or current repository context, classify the OpenCode session as poisoned for this workflow and stop. Do not retry in the same session.
